 ...
 * @status
 */
public class act_movie implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "act_movie.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(act_movie.class);
	...
	private final String jobName = "act_movie";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(act_movie.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row4Struct implements routines.system.IPersistableComparableLookupRow<row4Struct> {
		final static byte[] commonByteArrayLock_IMDB_act_movie = new byte[0];
		static byte[] commonByteArray_IMDB_act_movie = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_IMDB_act_movie.length) {
					if (length < 1024 && commonByteArray_IMDB_act_movie.length == 0) {
						commonByteArray_IMDB_act_movie = new byte[1024];
					} else {
						commonByteArray_IMDB_act_movie = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_IMDB_act_movie, 0, length);
				strReturn = new String(commonByteArray_IMDB_act_movie, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_IMDB_act_movie, 0, length);
				strReturn = new String(commonByteArray_IMDB_act_movie, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readKeysData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_IMDB_act_movie) {
	...
	public void tDBInput_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		org.slf4j.MDC.put("_subJobPid", "YM86EZ_" + subJobPidCounter.getAndIncrement());
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// linked node: tMap_1 - inputs:(row4,row6,row1,row8) outputs:(out1)
						...
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tDBInput_1
									.append("PASS" + " = "
											...
											+ "...");
							log4jParamters_tDBInput_1.append(" | ");
							log4jParamters_tDBInput_1.append("TABLE" + " = " + "\"stg_title_ratings\"");
							log4jParamters_tDBInput_1.append(" | ");
				...
				if (enableLogStash) {
					talendJobLog.addCM("tDBInput_1", "\"stg_title_ratings\"", "tMysqlInput");
					talendJobLogProcess(globalMap);
				}
				...
				try {
					...
					while (rs_tDBInput_1.next()) {
						...
						if (colQtyInRs_tDBInput_1 < 1) {
							...
							row4.tconst = routines.system.JDBCUtil.getString(rs_tDBInput_1, 1, false);
						}
						if (colQtyInRs_tDBInput_1 < 2) {
							row4.averageRating = null;
						} else {

							row4.averageRating = rs_tDBInput_1.getDouble(2);
						...
						if (colQtyInRs_tDBInput_1 < 3) {
							...
							row4.numVotes = rs_tDBInput_1.getInt(3);
							if (rs_tDBInput_1.wasNull()) {
								row4.numVotes = null;
							}
						}
						if (colQtyInRs_tDBInput_1 < 4) {
							...
							if (rs_tDBInput_1.getString(4) != null) {
								...
								if (!("0000-00-00").equals(dateString_tDBInput_1)
									...
									row4.DI_CreatedDate = rs_tDBInput_1.getTimestamp(4);
								} else {
									row4.DI_CreatedDate = (java.util.Date) year0_tDBInput_1.clone();
								}
							} else {
								row4.DI_CreatedDate = null;
							}
						...
						if (colQtyInRs_tDBInput_1 < 5) {
							...
							row4.DI_JobID = routines.system.JDBCUtil.getString(rs_tDBInput_1, 5, false);
						}
						...
						if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
								...
								, "row4", "tDBInput_1", "\"stg_title_ratings\"", "tMysqlInput", "tAdvancedHash_row4",
						...
						row4_HashRow.averageRating = row4.averageRating;

						row4_HashRow.numVotes = row4.numVotes;
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row4", 2, 0,
						"tDBInput_1", "\"stg_title_ratings\"", "tMysqlInput", "tAdvancedHash_row4",
	...
	public static class row6Struct implements routines.system.IPersistableComparableLookupRow<row6Struct> {
		...
		public void readValuesData(DataInputStream dis, ObjectInputStream ois) {
			try {
				...
				this.isAdult = readInteger(dis, ois);
		...
		public void readValuesData(DataInputStream dis, org.jboss.marshalling.Unmarshaller objectIn) {
			try {
				...
				this.isAdult = readInteger(dis, objectIn);
		...
		public void writeValuesData(DataOutputStream dos, ObjectOutputStream oos) {
			try {
				...
				writeInteger(this.isAdult, dos, oos);
		...
		public void writeValuesData(DataOutputStream dos, org.jboss.marshalling.Marshaller objectOut) {
			try {
				...
				writeInteger(this.isAdult, dos, objectOut);
	...
	public void tDBInput_2Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		org.slf4j.MDC.put("_subJobPid", "Ze3rGM_" + subJobPidCounter.getAndIncrement());
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// source node:tUnite_1 - inputs:(out3,out2) outputs:(row6,row6) | target
				...
				try {
					...
					while (rs_tDBInput_2.next()) {
						...
						if (colQtyInRs_tDBInput_2 < 2) {
							...
							row3.titleType = routines.system.JDBCUtil.getString(rs_tDBInput_2, 2, false);
						}
						if (colQtyInRs_tDBInput_2 < 3) {
							row3.primaryTitle = null;
						} else {

							row3.primaryTitle = routines.system.JDBCUtil.getString(rs_tDBInput_2, 3, false);
						}
						if (colQtyInRs_tDBInput_2 < 4) {
							row3.originalTitle = null;
						} else {

							row3.originalTitle = routines.system.JDBCUtil.getString(rs_tDBInput_2, 4, false);
						}
						if (colQtyInRs_tDBInput_2 < 5) {
							row3.isAdult = null;
						} else {

							row3.isAdult = rs_tDBInput_2.getInt(5);
						...
						if (colQtyInRs_tDBInput_2 < 6) {
							...
							row3.startYear = routines.system.JDBCUtil.getString(rs_tDBInput_2, 6, false);
						}
						if (colQtyInRs_tDBInput_2 < 7) {
							row3.endYear = null;
						} else {

							row3.endYear = routines.system.JDBCUtil.getString(rs_tDBInput_2, 7, false);
						}
						if (colQtyInRs_tDBInput_2 < 8) {
							row3.runtimeMinutes = null;
						} else {

							row3.runtimeMinutes = rs_tDBInput_2.getInt(8);
							if (rs_tDBInput_2.wasNull()) {
								row3.runtimeMinutes = null;
							}
						}
						...
						if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
								...
								, "row3", "tDBInput_2", "\"title_basics_stg\"", "tMysqlInput", "tMap_3", "tMap_3",
								"tMap"
...
// # Output table : 'out3'
							...
							out3_tmp.tconst = row3.tconst;
							out3_tmp.titleType = row3.titleType;
							out3_tmp.primaryTitle = row3.primaryTitle;
							out3_tmp.originalTitle = row3.originalTitle;
							out3_tmp.isAdult = row3.isAdult;
							out3_tmp.startYear = row3.startYear;
							out3_tmp.endYear = row3.endYear;
							out3_tmp.runtimeMinutes = row3.runtimeMinutes;
							out3 = out3_tmp;
...
// Start of branch "out3"
						if (out3 != null) {
							...
							if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
									...
									, "out3", "tMap_3", "tMap_3", "tMap", "tUnite_1", "tUnite_1", "tUnite"
...
// for output
							...
							row6.tconst = out3.tconst;
							row6.titleType = out3.titleType;
							row6.primaryTitle = out3.primaryTitle;
							row6.originalTitle = out3.originalTitle;
							row6.isAdult = out3.isAdult;
							row6.startYear = out3.startYear;
							row6.endYear = out3.endYear;
							row6.runtimeMinutes = out3.runtimeMinutes;
...
//////////
							...
							if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
									...
									, "row6", "tUnite_1", "tUnite_1", "tUnite", "tAdvancedHash_row6",
							...
							row6_HashRow.tconst = row6.tconst;

							row6_HashRow.titleType = row6.titleType;

							row6_HashRow.primaryTitle = row6.primaryTitle;

							row6_HashRow.originalTitle = row6.originalTitle;

							row6_HashRow.isAdult = row6.isAdult;

							row6_HashRow.startYear = row6.startYear;

							row6_HashRow.endYear = row6.endYear;

							row6_HashRow.runtimeMinutes = row6.runtimeMinutes;
...
// ###############################      
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row3", 2, 0,
						"tDBInput_2", "\"title_basics_stg\"", "tMysqlInput", "tMap_3", "tMap_3", "tMap", "output")) {
					talendJobLogProcess(globalMap);
				}
...
// ###############################
						...
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tDBInput_3
									.append("PASS" + " = "
											...
											+ "...");
							log4jParamters_tDBInput_3.append(" | ");
							log4jParamters_tDBInput_3.append("TABLE" + " = " + "\"stg_title_basics\"");
							log4jParamters_tDBInput_3.append(" | ");
				...
				if (enableLogStash) {
					talendJobLog.addCM("tDBInput_3", "\"stg_title_basics\"", "tMysqlInput");
					talendJobLogProcess(globalMap);
				}
				...
				try {
					...
					while (rs_tDBInput_3.next()) {
						...
						if (colQtyInRs_tDBInput_3 < 1) {
							...
							row2.tconst = routines.system.JDBCUtil.getString(rs_tDBInput_3, 1, false);
						}
						if (colQtyInRs_tDBInput_3 < 2) {
							row2.titleType = null;
						} else {

							row2.titleType = routines.system.JDBCUtil.getString(rs_tDBInput_3, 2, false);
						}
						if (colQtyInRs_tDBInput_3 < 3) {
							row2.primaryTitle = null;
						} else {

							row2.primaryTitle = routines.system.JDBCUtil.getString(rs_tDBInput_3, 3, false);
						}
						if (colQtyInRs_tDBInput_3 < 4) {
							row2.originalTitle = null;
						} else {

							row2.originalTitle = routines.system.JDBCUtil.getString(rs_tDBInput_3, 4, false);
						}
						if (colQtyInRs_tDBInput_3 < 5) {
							row2.isAdult = null;
						} else {

							row2.isAdult = rs_tDBInput_3.getByte(5);
						...
						if (colQtyInRs_tDBInput_3 < 6) {
							...
							row2.startYear = routines.system.JDBCUtil.getString(rs_tDBInput_3, 6, false);
						}
						if (colQtyInRs_tDBInput_3 < 7) {
							row2.endYear = null;
						} else {

							row2.endYear = routines.system.JDBCUtil.getString(rs_tDBInput_3, 7, false);
						}
						if (colQtyInRs_tDBInput_3 < 8) {
							row2.runtimeMinutes = null;
						} else {

							row2.runtimeMinutes = rs_tDBInput_3.getInt(8);
							if (rs_tDBInput_3.wasNull()) {
								row2.runtimeMinutes = null;
							}
						}
						if (colQtyInRs_tDBInput_3 < 9) {
							...
							if (rs_tDBInput_3.getString(9) != null) {
								...
								if (!("0000-00-00").equals(dateString_tDBInput_3)
									...
									row2.DI_CreatedDate = rs_tDBInput_3.getTimestamp(9);
								} else {
									row2.DI_CreatedDate = (java.util.Date) year0_tDBInput_3.clone();
								}
							} else {
								row2.DI_CreatedDate = null;
							}
						...
						if (colQtyInRs_tDBInput_3 < 10) {
							...
							row2.DI_JobID = routines.system.JDBCUtil.getString(rs_tDBInput_3, 10, false);
						}
						...
						if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
								...
								, "row2", "tDBInput_3", "\"stg_title_basics\"", "tMysqlInput", "tConvertType_1",
						...
						try {
							row7.tconst = TypeConvert.String2String(row2.tconst);
						} catch (java.lang.Exception e) {
						...
						try {
							row7.titleType = TypeConvert.String2String(row2.titleType);
						} catch (java.lang.Exception e) {
							globalMap.put("tConvertType_1_ERROR_MESSAGE", e.getMessage());
							bHasError_tConvertType_1 = true;
							System.err.println(e.getMessage());
						...
						try {
							row7.primaryTitle = TypeConvert.String2String(row2.primaryTitle);
						} catch (java.lang.Exception e) {
							globalMap.put("tConvertType_1_ERROR_MESSAGE", e.getMessage());
							bHasError_tConvertType_1 = true;
							System.err.println(e.getMessage());
						...
						try {
							row7.originalTitle = TypeConvert.String2String(row2.originalTitle);
						} catch (java.lang.Exception e) {
							globalMap.put("tConvertType_1_ERROR_MESSAGE", e.getMessage());
							bHasError_tConvertType_1 = true;
							System.err.println(e.getMessage());
						...
						try {
							row7.isAdult = TypeConvert.Byte2Integer(row2.isAdult);
						} catch (java.lang.Exception e) {
							globalMap.put("tConvertType_1_ERROR_MESSAGE", e.getMessage());
							bHasError_tConvertType_1 = true;
							System.err.println(e.getMessage());
						...
						try {
							row7.startYear = TypeConvert.String2String(row2.startYear);
						} catch (java.lang.Exception e) {
							globalMap.put("tConvertType_1_ERROR_MESSAGE", e.getMessage());
							bHasError_tConvertType_1 = true;
							System.err.println(e.getMessage());
						...
						try {
							row7.endYear = TypeConvert.String2String(row2.endYear);
						} catch (java.lang.Exception e) {
							globalMap.put("tConvertType_1_ERROR_MESSAGE", e.getMessage());
							bHasError_tConvertType_1 = true;
							System.err.println(e.getMessage());
						...
						try {
							row7.runtimeMinutes = TypeConvert.Integer2Integer(row2.runtimeMinutes);
						} catch (java.lang.Exception e) {
							globalMap.put("tConvertType_1_ERROR_MESSAGE", e.getMessage());
							bHasError_tConvertType_1 = true;
							System.err.println(e.getMessage());
						...
						try {
							row7.DI_CreatedDate = TypeConvert.Date2Date(row2.DI_CreatedDate);
						} catch (java.lang.Exception e) {
							globalMap.put("tConvertType_1_ERROR_MESSAGE", e.getMessage());
							bHasError_tConvertType_1 = true;
							System.err.println(e.getMessage());
						...
						try {
							row7.DI_JobID = TypeConvert.String2String(row2.DI_JobID);
						} catch (java.lang.Exception e) {
							globalMap.put("tConvertType_1_ERROR_MESSAGE", e.getMessage());
							bHasError_tConvertType_1 = true;
							System.err.println(e.getMessage());
...
// Start of branch "row7"
						if (row7 != null) {
							...
							if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
									...
									, "row7", "tConvertType_1", "tConvertType_1", "tConvertType", "tMap_2", "tMap_2",
									"tMap"
...
// # Output table : 'out2'
								...
								out2_tmp.tconst = row7.tconst;
								out2_tmp.titleType = row7.titleType;
								out2_tmp.primaryTitle = row7.primaryTitle;
								out2_tmp.originalTitle = row7.originalTitle;
								out2_tmp.isAdult = row7.isAdult;
								out2_tmp.startYear = row7.startYear;
								out2_tmp.endYear = row7.endYear;
								out2_tmp.runtimeMinutes = row7.runtimeMinutes;
								out2 = out2_tmp;
...
// for output
								...
								row6.tconst = out2.tconst;
								row6.titleType = out2.titleType;
								row6.primaryTitle = out2.primaryTitle;
								row6.originalTitle = out2.originalTitle;
								row6.isAdult = out2.isAdult;
								row6.startYear = out2.startYear;
								row6.endYear = out2.endYear;
								row6.runtimeMinutes = out2.runtimeMinutes;
...
//////////
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row2", 2, 0,
						"tDBInput_3", "\"stg_title_basics\"", "tMysqlInput", "tConvertType_1", "tConvertType_1",
						"tConvertType", "output")) {
					talendJobLogProcess(globalMap);
				}
...
// ###############################      
				...
				if (enableLogStash) {
					...
					if (runStat.log(resourceMap, iterateId, "out3", 2, 0, "tMap_3", "tMap_3", "tMap", "tUnite_1",
							"tUnite_1", "tUnite", "output")) {
						talendJobLogProcess(globalMap);
					}
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row6", 2, 0, "tUnite_1",
						"tUnite_1", "tUnite", "tAdvancedHash_row6", "tAdvancedHash_row6", "tAdvancedHash", "output")) {
					talendJobLogProcess(globalMap);
				}
	...
	public static class out1Struct implements routines.system.IPersistableRow<out1Struct> {
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_IMDB_act_movie) {
				...
				try {
					...
					this.no_of_votes = readInteger(dis);

					this.title_sk = dis.readInt();

					this.WorldWideGross = readInteger(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeInteger(this.no_of_votes, dos);
				...
				writeInteger(this.WorldWideGross, dos);
	...
	public static class after_tDBInput_4Struct implements routines.system.IPersistableRow<after_tDBInput_4Struct> {
		...
		public int hashCode() {
			if (this.hashCodeDirty) {
				...
				result = prime * result + (int) this.title_sk;
		...
		public boolean equals(Object obj) {
			...
			if (this.title_sk != other.title_sk)
				return false;
		...
		public int compareTo(after_tDBInput_4Struct other) {
			...
			returnValue = checkNullsAndCompare(this.title_sk, other.title_sk);
			if (returnValue != 0) {
				return returnValue;
			}
	...
	public void tDBInput_4Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		org.slf4j.MDC.put("_subJobPid", "t0Wwru_" + subJobPidCounter.getAndIncrement());
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
					...
					stmtCreate_tDBOutput_1.execute("CREATE TABLE `" + tableName_tDBOutput_1
							+ "`(`movie_id` INT(0)  ,`tconst` VARCHAR(10)   not null ,`run_time` INT(10)  default 0 ,`rating` DOUBLE(22,0)  ,`no_of_votes` INT(10)  ,`title_sk` INT(10)   not null ,`WorldWideGross` INT(10)  ,`release_year` VARCHAR(4)  ,`DI_CreatedDate` DATETIME  not null ,`DI_JobID` VARCHAR(10)   not null ,primary key(`movie_id`))");
					if (log.isDebugEnabled())
				...
				java.sql.PreparedStatement pstmt_tDBOutput_1 = conn_tDBOutput_1
						.prepareStatement("SELECT COUNT(1) FROM `" + "fact_movie" + "` WHERE `movie_id` = ?");
				resourceMap.put("pstmt_tDBOutput_1", pstmt_tDBOutput_1);
				String insert_tDBOutput_1 = "INSERT INTO `" + "fact_movie"
						+ "` (`movie_id`,`tconst`,`run_time`,`rating`,`no_of_votes`,`title_sk`,`WorldWideGross`,`release_year`,`DI_CreatedDate`,`DI_JobID`) VALUES (?,?,?,?,?,?,?,?,?,?)";

				java.sql.PreparedStatement pstmtInsert_tDBOutput_1 = conn_tDBOutput_1
						.prepareStatement(insert_tDBOutput_1);
				resourceMap.put("pstmtInsert_tDBOutput_1", pstmtInsert_tDBOutput_1);
				String update_tDBOutput_1 = "UPDATE `" + "fact_movie"
						+ "` SET `tconst` = ?,`run_time` = ?,`rating` = ?,`no_of_votes` = ?,`title_sk` = ?,`WorldWideGross` = ?,`release_year` = ?,`DI_CreatedDate` = ?,`DI_JobID` = ? WHERE `movie_id` = ?";
...
// ###############################
						...
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tDBInput_4
									.append("PASS" + " = "
											...
											+ "...");
							log4jParamters_tDBInput_4.append(" | ");
							log4jParamters_tDBInput_4.append("TABLE" + " = " + "\"title_dim\"");
							log4jParamters_tDBInput_4.append(" | ");
							log4jParamters_tDBInput_4.append("QUERYSTORE" + " = " + "\"\"");
							log4jParamters_tDBInput_4.append(" | ");
							log4jParamters_tDBInput_4.append("QUERY" + " = "
									+ "\"SELECT    `title_dim`.`tconst`,    `title_dim`.`title`,    `title_dim`.`release_year`,    `title_dim`.`title_sk`,    `title_dim`.`scd_start`,    `title_dim`.`scd_end`,    `title_dim`.`scd_version`,    `title_dim`.`scd_active` FROM `title_dim`\"");
							log4jParamters_tDBInput_4.append(" | ");
							...
							log4jParamters_tDBInput_4.append("TRIM_COLUMN" + " = " + "[{TRIM=" + ("false")
									+ ", SCHEMA_COLUMN=" + ("tconst") + "}, {TRIM=" + ("false") + ", SCHEMA_COLUMN="
									+ ("title") + "}, {TRIM=" + ("false") + ", SCHEMA_COLUMN=" + ("release_year")
									+ "}, {TRIM=" + ("false") + ", SCHEMA_COLUMN=" + ("title_sk") + "}, {TRIM="
				...
				if (enableLogStash) {
					talendJobLog.addCM("tDBInput_4", "\"title_dim\"", "tMysqlInput");
					talendJobLogProcess(globalMap);
				}
				...
				String dbquery_tDBInput_4 = "SELECT \n  `title_dim`.`tconst`, \n  `title_dim`.`title`, \n  `title_dim`.`release_year`, \n  `title_dim`.`title_sk`, \n  `t"
						+ "itle_dim`.`scd_start`, \n  `title_dim`.`scd_end`, \n  `title_dim`.`scd_version`, \n  `title_dim`.`scd_active`\nFROM `title_d"
						+ "im`";
				...
				try {
					...
					while (rs_tDBInput_4.next()) {
						...
						if (colQtyInRs_tDBInput_4 < 2) {
							...
							row1.title = routines.system.JDBCUtil.getString(rs_tDBInput_4, 2, false);
						}
						if (colQtyInRs_tDBInput_4 < 3) {
							row1.release_year = null;
						} else {

							row1.release_year = routines.system.JDBCUtil.getString(rs_tDBInput_4, 3, false);
						}
						if (colQtyInRs_tDBInput_4 < 4) {
							row1.title_sk = 0;
						} else {

							row1.title_sk = rs_tDBInput_4.getInt(4);
						...
						if (colQtyInRs_tDBInput_4 < 5) {
							...
							if (rs_tDBInput_4.getString(5) != null) {
								...
								if (!("0000-00-00").equals(dateString_tDBInput_4)
									...
									row1.scd_start = rs_tDBInput_4.getTimestamp(5);
								} else {
									row1.scd_start = (java.util.Date) year0_tDBInput_4.clone();
								}
							} else {
								row1.scd_start = null;
							}
						...
						if (colQtyInRs_tDBInput_4 < 6) {
							...
							if (rs_tDBInput_4.getString(6) != null) {
								...
								if (!("0000-00-00").equals(dateString_tDBInput_4)
										&& !("0000-00-00 00:00:00").equals(dateString_tDBInput_4)) {
									row1.scd_end = rs_tDBInput_4.getTimestamp(6);
								} else {
									row1.scd_end = (java.util.Date) year0_tDBInput_4.clone();
								}
							} else {
								row1.scd_end = null;
							}
						...
						if (colQtyInRs_tDBInput_4 < 7) {
							...
							row1.scd_version = rs_tDBInput_4.getInt(7);
							if (rs_tDBInput_4.wasNull()) {
								throw new RuntimeException("Null value in non-Nullable column");
							}
						}
						if (colQtyInRs_tDBInput_4 < 8) {
							...
							row1.scd_active = rs_tDBInput_4.getBoolean(8);
							if (rs_tDBInput_4.wasNull()) {
								throw new RuntimeException("Null value in non-Nullable column");
							}
						}
						...
						if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
								...
								, "row1", "tDBInput_4", "\"title_dim\"", "tMysqlInput", "tMap_1", "tMap_1", "tMap"
						...
						if (!rejectedInnerJoin_tMap_1) { // G_TM_M_020
							...
							row6HashKey.tconst = row1.tconst;
						...
						if (tHash_Lookup_row6 != null && tHash_Lookup_row6.getCount(row6HashKey) > 1) { // G 071
							...
							// and it contains more one result from keys : row6.tconst = '" +
							// row6HashKey.tconst + "'");
						} // G 071
						...
						if (!rejectedInnerJoin_tMap_1) { // G_TM_M_020
							...
							row8HashKey.tconst = row1.tconst;
						...
						if (tHash_Lookup_row8 != null && tHash_Lookup_row8.getCount(row8HashKey) > 1) { // G 071
							...
							// and it contains more one result from keys : row8.tconst = '" +
							// row8HashKey.tconst + "'");
						} // G 071
...
// # Output table : 'out1'
							...
							out1_tmp.movie_id = Numeric.sequence("s251", 1, 1);
							out1_tmp.tconst = row1.tconst;
							out1_tmp.run_time = row6.runtimeMinutes;
							out1_tmp.rating = row4.averageRating;
							out1_tmp.no_of_votes = row4.numVotes;
							out1_tmp.title_sk = row1.title_sk;
							out1_tmp.WorldWideGross = row8.WorldWideGross;
							out1_tmp.release_year = row1.release_year;
						...
						if (out1 != null) {
							...
							if (checkCount_tDBOutput_1 > 0) {
								if (out1.tconst == null) {
									...
									pstmtUpdate_tDBOutput_1.setString(1, out1.tconst);
								}

								if (out1.run_time == null) {
									pstmtUpdate_tDBOutput_1.setNull(2, java.sql.Types.INTEGER);
								} else {
									pstmtUpdate_tDBOutput_1.setInt(2, out1.run_time);
								}

								if (out1.rating == null) {
									pstmtUpdate_tDBOutput_1.setNull(3, java.sql.Types.DOUBLE);
								} else {
									pstmtUpdate_tDBOutput_1.setDouble(3, out1.rating);
								}

								if (out1.no_of_votes == null) {
									pstmtUpdate_tDBOutput_1.setNull(4, java.sql.Types.INTEGER);
								} else {
									pstmtUpdate_tDBOutput_1.setInt(4, out1.no_of_votes);
								}

								pstmtUpdate_tDBOutput_1.setInt(5, out1.title_sk);

								if (out1.WorldWideGross == null) {
									pstmtUpdate_tDBOutput_1.setNull(6, java.sql.Types.INTEGER);
								} else {
									pstmtUpdate_tDBOutput_1.setInt(6, out1.WorldWideGross);
								}

								if (out1.release_year == null) {
									pstmtUpdate_tDBOutput_1.setNull(7, java.sql.Types.VARCHAR);
								} else {
									pstmtUpdate_tDBOutput_1.setString(7, out1.release_year);
								}
								...
								if (out1.DI_JobID == null) {
									...
									pstmtUpdate_tDBOutput_1.setString(9, out1.DI_JobID);
								}

								if (out1.movie_id == null) {
									pstmtUpdate_tDBOutput_1.setNull(10 + count_tDBOutput_1, java.sql.Types.INTEGER);
								} else {
									pstmtUpdate_tDBOutput_1.setInt(10 + count_tDBOutput_1, out1.movie_id);
								}
								...
								if (out1.movie_id == null) {
									...
									pstmtInsert_tDBOutput_1.setInt(1, out1.movie_id);
								}

								if (out1.tconst == null) {
									pstmtInsert_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
								} else {
									pstmtInsert_tDBOutput_1.setString(2, out1.tconst);
								}

								if (out1.run_time == null) {
									pstmtInsert_tDBOutput_1.setNull(3, java.sql.Types.INTEGER);
								} else {
									pstmtInsert_tDBOutput_1.setInt(3, out1.run_time);
								}

								if (out1.rating == null) {
									pstmtInsert_tDBOutput_1.setNull(4, java.sql.Types.DOUBLE);
								} else {
									pstmtInsert_tDBOutput_1.setDouble(4, out1.rating);
								}

								if (out1.no_of_votes == null) {
									pstmtInsert_tDBOutput_1.setNull(5, java.sql.Types.INTEGER);
								} else {
									pstmtInsert_tDBOutput_1.setInt(5, out1.no_of_votes);
								}

								pstmtInsert_tDBOutput_1.setInt(6, out1.title_sk);

								if (out1.WorldWideGross == null) {
									pstmtInsert_tDBOutput_1.setNull(7, java.sql.Types.INTEGER);
								} else {
									pstmtInsert_tDBOutput_1.setInt(7, out1.WorldWideGross);
								}

								if (out1.release_year == null) {
									pstmtInsert_tDBOutput_1.setNull(8, java.sql.Types.VARCHAR);
								} else {
									pstmtInsert_tDBOutput_1.setString(8, out1.release_year);
								}
								...
								if (out1.DI_JobID == null) {
									...
									pstmtInsert_tDBOutput_1.setString(10, out1.DI_JobID);
								}
...
// ###############################      
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row1", 2, 0,
						"tDBInput_4", "\"title_dim\"", "tMysqlInput", "tMap_1", "tMap_1", "tMap", "output")) {
					talendJobLogProcess(globalMap);
				}
	...
	public void tDBInput_5Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		org.slf4j.MDC.put("_subJobPid", "R1Er0S_" + subJobPidCounter.getAndIncrement());
					...
					class BytesLimit65535_tAggregateRow_1_AGGOUT {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tAggregateRow_1_AGGOUT.append("GROUPBYS" + " = " + "[{OUTPUT_COLUMN="
									+ ("tconst") + ", INPUT_COLUMN=" + ("tconst") + "}]");
							log4jParamters_tAggregateRow_1_AGGOUT.append(" | ");
							log4jParamters_tAggregateRow_1_AGGOUT.append("OPERATIONS" + " = " + "[{OUTPUT_COLUMN="
									+ ("WorldWideGross") + ", INPUT_COLUMN=" + ("Gross") + ", IGNORE_NULL=" + ("false")
									+ ", FUNCTION=" + ("sum") + "}]");
							log4jParamters_tAggregateRow_1_AGGOUT.append(" | ");
...
// ------------
						...
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tDBInput_5
									.append("PASS" + " = "
											...
											+ "...");
							log4jParamters_tDBInput_5.append(" | ");
							log4jParamters_tDBInput_5.append("TABLE" + " = " + "\"movie_revenuefinal\"");
							log4jParamters_tDBInput_5.append(" | ");
				...
				if (enableLogStash) {
					talendJobLog.addCM("tDBInput_5", "\"movie_revenuefinal\"", "tMysqlInput");
					talendJobLogProcess(globalMap);
				}
				...
				try {
					...
					while (rs_tDBInput_5.next()) {
						...
						if (colQtyInRs_tDBInput_5 < 1) {
							...
							row5.tconst = routines.system.JDBCUtil.getString(rs_tDBInput_5, 1, false);
						}
						...
						if (colQtyInRs_tDBInput_5 < 3) {
							...
							if (rs_tDBInput_5.getString(3) != null) {
								...
								if (!("0000-00-00").equals(dateString_tDBInput_5)
									...
									row5.Date = rs_tDBInput_5.getTimestamp(3);
								} else {
									row5.Date = (java.util.Date) year0_tDBInput_5.clone();
								}
							} else {
								row5.Date = null;
							}
						...
						if (colQtyInRs_tDBInput_5 < 4) {
							...
							row5.Rank = routines.system.JDBCUtil.getString(rs_tDBInput_5, 4, false);
						}
						if (colQtyInRs_tDBInput_5 < 5) {
							row5.Gross = null;
						} else {

							row5.Gross = rs_tDBInput_5.getInt(5);
						...
						if (colQtyInRs_tDBInput_5 < 6) {
							...
							row5._YD = rs_tDBInput_5.getDouble(6);
							if (rs_tDBInput_5.wasNull()) {
								row5._YD = null;
							}
						}
						if (colQtyInRs_tDBInput_5 < 7) {
							...
							row5._LW = rs_tDBInput_5.getDouble(7);
							if (rs_tDBInput_5.wasNull()) {
								row5._LW = null;
							}
						}
						if (colQtyInRs_tDBInput_5 < 8) {
							...
							row5.Theaters = rs_tDBInput_5.getInt(8);
							if (rs_tDBInput_5.wasNull()) {
								row5.Theaters = null;
							}
						}
						if (colQtyInRs_tDBInput_5 < 9) {
							row5.Per_Theater = null;
						} else {

							row5.Per_Theater = rs_tDBInput_5.getInt(9);
							if (rs_tDBInput_5.wasNull()) {
								row5.Per_Theater = null;
							}
						}
						if (colQtyInRs_tDBInput_5 < 10) {
							row5.Total_Gross = null;
						} else {

							row5.Total_Gross = rs_tDBInput_5.getInt(10);
							if (rs_tDBInput_5.wasNull()) {
								row5.Total_Gross = null;
							}
						}
						if (colQtyInRs_tDBInput_5 < 11) {
							...
							row5.Days = rs_tDBInput_5.getInt(11);
							if (rs_tDBInput_5.wasNull()) {
								row5.Days = null;
							}
						}
						if (colQtyInRs_tDBInput_5 < 12) {
							row5.DI_Filename = null;
						} else {

							row5.DI_Filename = routines.system.JDBCUtil.getString(rs_tDBInput_5, 12, false);
						}
						if (colQtyInRs_tDBInput_5 < 13) {
							row5.DI_CreatedDate = null;
						} else {
							...
							if (rs_tDBInput_5.getString(13) != null) {
								...
								if (!("0000-00-00").equals(dateString_tDBInput_5)
										&& !("0000-00-00 00:00:00").equals(dateString_tDBInput_5)) {
									row5.DI_CreatedDate = rs_tDBInput_5.getTimestamp(13);
								} else {
									row5.DI_CreatedDate = (java.util.Date) year0_tDBInput_5.clone();
								}
							} else {
								row5.DI_CreatedDate = null;
							}
						...
						if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
								...
								, "row5", "tDBInput_5", "\"movie_revenuefinal\"", "tMysqlInput",
						...
						operation_finder_tAggregateRow_1.tconst = row5.tconst;
						...
						if (operation_result_tAggregateRow_1.WorldWideGross_sum == null) {
							operation_result_tAggregateRow_1.WorldWideGross_sum = (int) 0;
						}

						if (row5.Gross != null)
							operation_result_tAggregateRow_1.WorldWideGross_sum += row5.Gross;
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row5", 2, 0,
						"tDBInput_5", "\"movie_revenuefinal\"", "tMysqlInput", "tAggregateRow_1_AGGOUT",
				...
				for (AggOperationStruct_tAggregateRow_1 aggregated_row_tAggregateRow_1 : values_tAggregateRow_1) { // G_AggR_600
					...
					row8.tconst = aggregated_row_tAggregateRow_1.tconst;
					row8.WorldWideGross = aggregated_row_tAggregateRow_1.WorldWideGross_sum;

					if (log.isDebugEnabled())
						log.debug("tAggregateRow_1_AGGIN - " + ("Operation function: 'sum' on the column 'Gross'."));
					...
					if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
							...
							, "row8", "tAggregateRow_1_AGGIN", "tAggregateRow_1_AGGIN", "tAggregateIn",
					...
					row8_HashRow.tconst = row8.tconst;

					row8_HashRow.WorldWideGross = row8.WorldWideGross;
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row8", 2, 0,
						"tAggregateRow_1_AGGIN", "tAggregateRow_1_AGGIN", "tAggregateIn", "tAdvancedHash_row8",
	...
	public void talendJobLogProcess(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		org.slf4j.MDC.put("_subJobPid", "h0oYTi_" + subJobPidCounter.getAndIncrement());
	...
	public static void main(String[] args) {
		final act_movie act_movieClass = new act_movie();

		int exitCode = act_movieClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'act_movie' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		getjobInfo();
		log.info("TalendJob: 'act_movie' - Start.");
		...
		org.slf4j.MDC.put("_jobRepositoryId", "_cwzyoJKIEe6_WIiIWRIzPA");
		org.slf4j.MDC.put("_compiledAtTimestamp", "2023-12-04T22:12:49.894053300Z");
		...
		try {
			...
			java.io.InputStream inContext = act_movie.class.getClassLoader()
					.getResourceAsStream("imdb/act_movie_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = act_movie.class.getClassLoader()
		...
		log.info("TalendJob: 'act_movie' - Started.");
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : act_movie");
		}
		if (enableLogStash) {
		...
		log.info("TalendJob: 'act_movie' - Finished - status: " + status + " returnCode: " + returnCode);
...
/************************************************************************************************
 * 441600 characters generated by Talend Cloud Data Fabric on the 4 December
 * 2023 at 5:12:49 PM GMT-05:00
 ************************************************************************************************/